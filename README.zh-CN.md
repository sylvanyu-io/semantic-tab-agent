...
完成 AI 配置后，首次引导会默认选择完整网址和页面摘要增强，以提供更完整的整理与回顾结果，并在继续前显示红色警示。你可以改成只发标题或关闭页面文字。Chrome 会先询问站点权限；TabRecap 只读取已授权、未休眠、非无痕页面的少量可见文字。页面短摘要只保存在这台电脑，只有开始整理或回顾时，所选数据才会发送到你配置的 AI API。密码、表单内容、Cookie、本地存储和完整 HTML 不在读取范围内。没有授权的页面继续只使用标题和网址。
...
TabRecap 不内置服务商密钥。首次配置会从可选的体验接口和模型开始，并显示红色警示；你也可以在继续前换成任意兼容 OpenAI Chat Completions 的 Base URL 和模型 ID。公开接口或本机接口可以不填 API Key。点 **读取模型** 会请求该地址的 `/models`；接口不支持模型列表时，仍可手动填写。
...
首次配置的 AI 步骤会从这个地址和 `deepseek-v4-flash` 开始，并显示红色体验警示；体验接口不需要客户端 Key，所以 Key 默认留空。继续前可以检查或替换这些值。体验请求会先经过 TabRecap 的 Cloudflare Worker，并且只转发给 OpenCode Go。上游 Key 只保存在 Worker Secret 中，不进入源码和安装包。
