...
async function setAdvancedSettingsOpen(page, open) {
...
async function configureMockGateway(page) {
  ...
  await setAdvancedSettingsOpen(page, false);
}
...
test("time recap mode renders a first-class recap surface", async ({ page }) => {
  ...
  await expect(page.locator('[data-date-display-for="recapFromDate"] .date-input-value')).toHaveText(/\d{4}\/\d{2}\/\d{2} \d{2}:\d{2}/);
  ...
  await expect(page.locator('[data-date-display-for="recapFromDate"] .date-input-value')).toContainText("00:00");
...
test("time recap fallback surfaces custom API timeouts as user-actionable copy", async ({ page }) => {
  await page.addInitScript(() => {
    const settings = {
      organizeMode: "current_window",
      targetWindowMode: "current_window",
          ...
          if (message.type === "activity:generateTimeRecap") {
            return {
              ...
              result: {
                ...
                recap: {
                  schema: "tab_recap_time_recap_v1",
                  headline: "这段时间主要在排查自定义模型。",
                  summary: "本机线索显示最近集中在自定义 API 和回顾体验。",
                  timeline: [],
                  themes: [],
                  followUps: [],
                  coverageNote: "已参考本机活动。"
...
test("time recap fallback hides unknown raw gateway errors even when they mention AI gateway", async ({ page }) => {
        ...
        sendMessage: async (message) => {
          ...
          if (message.type === "activity:generateTimeRecap") {
            return {
              ...
              result: {
                source: "local_fallback",
                error: "AI gateway upstream panic: request token=SECRET123 trace=raw-stack",
                input: { pages: [], coverage: { includedPages: 9, sampledEntries: 1 } },
                recap: {
                  schema: "tab_recap_time_recap_v1",
                  headline: "这段时间主要在调试稳定性。",
                  summary: "本机线索显示最近集中在网关和回顾体验。",
                  timeline: [],
                  themes: [],
                  followUps: [],
                  coverageNote: "已参考本机活动。"
  ...
  await expect(page.locator("#timeRecapPanel")).not.toContainText("raw-stack");
});

test("time recap fallback surfaces friendly model errors without raw gateway copy", async ({ page }) => {
  await page.addInitScript(() => {
    const settings = {
      organizeMode: "current_window",
      targetWindowMode: "current_window",
          ...
          if (message.type === "activity:generateTimeRecap") {
            return {
              ...
              result: {
                ...
                recap: {
                  schema: "tab_recap_time_recap_v1",
                  headline: "这段时间主要在做模型路由。",
                  summary: "本机线索显示最近集中在扩展发布和模型测试。",
                  timeline: [],
                  themes: [],
                  followUps: [],
                  coverageNote: "已参考本机活动。"
...
test("cleanup candidates are returned with the generated plan and can be closed manually", async ({ page }) => {
  ...
  await expect(page.locator(".cleanup-preview")).not.toContainText("transitionCount");
...
test("side panel guides missing API setup before generation", async ({ page }) => {
  await page.addInitScript(() => {
    const settings = {
      organizeMode: "current_window",
      targetWindowMode: "current_window",
...
test("side panel restores a background planning error after reopening", async ({ page }) => {
  await page.addInitScript(() => {
    ...
    let activeJob = localStorage.getItem("tabRecapAnalysisCleared") === "1" ? null : {
    ...
    window.__analysisCleared = false;
    window.chrome = {
      runtime: {
        sendMessage: async (message) => {
          if (message.type === "settings:get") return { ok: true, result: settings };
          ...
          if (message.type === "tabs:clearAnalysisState") {
            window.__analysisCleared = true;
            localStorage.setItem("tabRecapAnalysisCleared", "1");
            activeJob = null;
            return { ok: true, result: { cleared: true } };
          }
          return { ok: true, result: null };
...
test("side panel translates stored core model errors", async ({ page }) => {
  await page.addInitScript(() => {
    const settings = {
      organizeMode: "current_window",
      targetWindowMode: "current_window",
    ...
    const activeJob = {
      operationId: "job_core_model_error",
      status: "error",
      phase: "error",
      progress: 100,
...
test("custom provider model errors use custom API copy", async ({ page }) => {
  await page.addInitScript(() => {
    ...
    const activeJob = {
      operationId: "job_custom_model_error",
      status: "error",
      phase: "error",
      progress: 100,
      message: "自定义 AI 网关这次没有完成请求（400）。model_not_allowed",
      error: "自定义 AI 网关这次没有完成请求（400）。model_not_allowed",
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
      finishedAt: new Date().toISOString()
    };
...
test("custom provider can use a manual model name", async ({ page }) => {
  await page.addInitScript(() => {
    const settings = {
      organizeMode: "current_window",
      targetWindowMode: "current_window",
      ...
      gatewayCustomModel: "deepseek-v4",
      gatewayAuxiliaryModel: "same_as_primary",
      gatewayCustomAuxiliaryModel: "gpt-5.4-mini",
      gatewayThinkingIntensity: "high",
      gatewayApiKey: "test-key",
      customPrompt: ""
    };
